...
这里的每个 Skill 都是 Agent 能直接加载的结构化指令集，遵循开放标准。无论是 Kilo、Claude Code 还是 Cursor、Windsurf，基本都能装。
...
| 🎨 [**ux-review（UX 体验审查）**](#-ux-reviewux-体验审查) | 不碰业务逻辑，专挑加载态、空态、异常边界等七大 UX 盲区，进行全维度的体验修复 |
...
这个 skill 的作用就是将一套**铁血纪律**强制注入到当前项目 AI 助手的系统提示词里（比如 `.kilo/KILO.md` 或 `.cursorrules` 等）。
...
> *"功能做完了，但体验很糙？加载没动画、报错就白屏。让 Agent 来做细节质检员。"*

只专注于前端交互体验，**绝对不碰任何底层核心业务逻辑**。它会像个强迫症产品经理一样，从 7 个维度把你的组件/页面审视一遍。
